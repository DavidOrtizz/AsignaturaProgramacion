...
public class Uno {
  ...
  public static boolean hayCartasEnMazo() {
    // Vamos uno por uno comprobanso una de las cartas es null
    for (int i = 0; i < NUM_CARTAS; i++) {
      if (cartas[i] == null) {
        return true;
      }
  ...
  public static Carta robaDelMazo() {
    // Este bucle for va a ir de uno en uno comprobando que no sea null
    for (int i = 0; i < NUM_CARTAS; i++) {
      if (cartas[i] != null) {
        // Cuando se encuentra lo va a imprimir y luego le decimos que su interior sea
        // null
        System.out.println(cartas[i].getNumero() + " " + cartas[i].getColor());
        cartas.getNumero = null;
        cartas.getColor = null;
        // Las cartas que son null se iran quedando a la izquierda mientras las cartas
        // que estan llenas se quedan a la derecha
      }
    }
  }
  ...
  public static void imprimeMesa() {
    ...
    System.out.print("\n\n"); // 2 Saltos de línea Imprime el mazo y la carta en juego, en 3 líneas
